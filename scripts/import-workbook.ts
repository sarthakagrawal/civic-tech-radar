...
if(workbookPath){
 ...
 const rows:unknown[]=[]; signalSheet.eachRow((r,i)=>{if(i===1)return;rows.push({row:i,date:text(r.getCell(1).value)||null,sourceType:text(r.getCell(3).value),source:text(r.getCell(4).value),actor:text(r.getCell(5).value),stage:text(r.getCell(6).value),title:text(r.getCell(7).value),link:text(r.getCell([122,123,124].includes(i)?10:11).value)});});
 ...
 const previous=JSON.parse(legacy.toString()).signals.rows.map((r:Record<string,string>,i:number)=>({row:i+2,date:r.date_observed||null,sourceType:r.source_type||'',source:r.source,actor:r.actor||'',stage:r.stage||'',title:r.title,link:r.link}));
 const {LegacyRows}=await import('./reconcile.js');await writeFile('data/legacy-input.json',JSON.stringify(LegacyRows.assert(previous),null,2)+'\n');
...
const legacyRows=JSON.parse(await readFile('data/legacy-input.json','utf8'));
const reconciliation=reconcile(input.rows,legacyRows);
await writeFile('public/exports/reconciliation.json',JSON.stringify(reconciliation,null,2)+'\n');
