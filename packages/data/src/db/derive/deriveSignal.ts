...
export const DERIVATION_VERSION='deterministic-v3';
...
export async function deriveFromRawEvent(db:PgliteDatabase<typeof schema>,raw:unknown){
 ...
 await tx.insert(schema.sources).values(sourceValues).onConflictDoNothing();
 ...
 if(actorId)await tx.insert(schema.actors).values({id:actorId,canonicalName:actorName!,aliases:input.actor?.aliases??[],createdAt:event.fetchedAt}).onConflictDoNothing();
 ...
 else if(event.fetchedAt>existing.derivedAt || (event.fetchedAt.getTime()===existing.derivedAt.getTime() && event.id>(existing.latestRawEventId??'')))await tx.update(schema.signals).set({stage:values.stage,summary:values.summary,whyItMattersVerbatim:values.whyItMattersVerbatim,tags:values.tags,derivationVersion:DERIVATION_VERSION,derivedAt:event.fetchedAt,latestRawEventId:event.id}).where(eq(schema.signals.id,signalId));
